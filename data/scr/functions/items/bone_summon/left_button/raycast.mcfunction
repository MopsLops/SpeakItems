execute as @e[type=skeleton,tag=!raycasting,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @e[dx=0] positioned ~0.99 ~0.99 ~0.99 run function scr:items/bone_summon/left_button/collide
...
execute if score .distance dist matches ..400 positioned ^ ^ ^0.05 rotated ~ ~ if block ~ ~ ~ #scr:raycast_pass run function scr:items/bone_summon/left_button/raycast
